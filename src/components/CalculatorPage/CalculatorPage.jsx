...
const DEFAULT_PALLET_HEIGHT = 15;
...
function CameraController({ mode, orbitRef }) {
  ...
  useFrame(() => {
    ...
    if (orbitRef.current) {
      ...
      const doneTarget =
        orbitRef.current.target.distanceTo(targetLook.current) < 0.02;
...
export default function CalculatorPage() {
  ...
  const utilPct = useMemo(() => {
    ...
    const boxVol = placedBoxes.reduce(
      (a, b) => a + b.dims.length * b.dims.width * b.dims.height,
      0,
    );
  ...
  const handleDeleteLevel = useCallback(
    (boxId) => {
      setBoxes((prev) => {
        ...
        if (!ref || ref.staged) return prev;

        const [, halfY] = getRotatedHalfExtents(ref.dims, ref.rotation);
        const baseY = ref.position?.[1] ?? halfY;
        const EPS = 0.005;

        return prev.filter((b) => {
          if (b.staged) return true;
          const [, bHalfY] = getRotatedHalfExtents(b.dims, b.rotation);
          const bBaseY = b.position?.[1] ?? bHalfY;
          return Math.abs(baseY - bBaseY) >= EPS;
        });
      });
      setSelectedId(null);
    },
  ...
  const handleFillLevel = useCallback(
    (boxId) => {
      setBoxes((prev) => {
        const ref = prev.find((b) => b.id === boxId);
        if (!ref) return prev;
        ...
        const [halfX, halfY, halfZ] = getRotatedHalfExtents(
          ref.dims,
          ref.rotation,
        );
        const baseY = ref.staged ? halfY : (ref.position?.[1] ?? halfY);
        if (baseY + halfY + palletHeightUnits > limitHeightUnits + 0.0005)
          return prev;
  ...
  const handleRotate = useCallback((id, axis, dir) => {
    ...
    setBoxes((prev) =>
      prev.map((b) => {
        ...
        // Swap 3D mapping to match 2D mental model:
        // axis "x" -> horizontal rotation (yaw) -> rot[1]
        // axis "y" -> vertical rotation (pitch) -> rot[0]
        rot[axis === "x" ? 1 : axis === "y" ? 0 : 2] += STEP * dir;
  ...
  const getRotateGlyph = useCallback((axis, dir) => {
    if (axis === "x") {
      // X = Horizontal rotation (Yaw-like)
      if (dir === 1) {
        return (
          <svg
            viewBox="0 0 24 24"
            ...
            height="18"
          >
            <path d="M 14 19 Q 5 12 14 5" />
            <polyline points="10 5 14 5 14 9" />
          </svg>
        );
      } else {
        return (
          <svg
            ...
            height="18"
          >
            <path d="M 14 5 Q 5 12 14 19" />
            <polyline points="10 19 14 19 14 15" />
          </svg>
        );
      }
    } else if (axis === "y") {
      // Y = Vertical rotation (Pitch-like)
      if (dir === 1) {
        return (
          <svg
            viewBox="0 0 24 24"
            ...
            height="18"
          >
            <path d="M 5 11 Q 12 20 19 11" />
            <polyline points="14 11 19 11 19 16" />
          </svg>
        );
      } else {
        return (
          <svg
            ...
            height="18"
          >
            <path d="M 19 11 Q 12 20 5 11" />
            <polyline points="10 11 5 11 5 16" />
          </svg>
        );
      }
    } else {
      ...
      if (dir === 1) {
        ...
        return (
          <svg
            ...
            height="18"
          >
            <path d="M 19 13 Q 12 4 5 13" />
            <polyline points="10 13 5 13 5 8" />
          </svg>
        );
      }
    }
      ...
      <div className="calc-canvas">
        ...
        {heightWarning ? (
          ...
          <div className="calc-hud">
            ...
            {stagedBoxes.length} staged · {placedBoxes.length} placed ·{" "}
            {maxLevels} levels · {utilPct}% utilized
          </div>
        )}
        ...
        {totalBoxes === 0 && (
          <div className="calc-empty">
            ...
            <div className="calc-empty-text">
              Set amount → click "Stage Boxes" to begin
            </div>
          </div>
        )}
        ...
        {contextMenu && (
          ...
          >
            ...
            <div className="calc-ctx-rot-grid">
              <button
                className="calc-ctx-rot-btn"
                onClick={() => {
                  handleRotate(contextMenu.boxId, "x", 1);
                  setContextMenu(null);
                }}
              >
                <span className="calc-ctx-rot-icon">
                  {getRotateGlyph("x", 1)}
                </span>
              ...
              <button
                className="calc-ctx-rot-btn"
                onClick={() => {
                  handleRotate(contextMenu.boxId, "y", 1);
                  setContextMenu(null);
                }}
              >
                <span className="calc-ctx-rot-icon">
              ...
              <button
                className="calc-ctx-rot-btn"
                onClick={() => {
                  handleRotate(contextMenu.boxId, "z", 1);
                  setContextMenu(null);
                }}
              >
                <span className="calc-ctx-rot-icon">
            ...
            <button
              className="calc-context-btn calc-context-btn-danger"
              onClick={() => {
                handleDeleteLevel(contextMenu.boxId);
                setContextMenu(null);
              }}
            >
              <span>✕</span> Delete all box in this level
            </button>
          </div>
        )}
